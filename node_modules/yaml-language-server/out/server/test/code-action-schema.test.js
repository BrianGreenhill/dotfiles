...
const testHelper_2 = require("./utils/testHelper");
describe('Schema Errors Code Action Tests', () => {
    ...
    describe('Convert value code action tests', () => {
        it('Should provide convert to boolean action for false', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const diagnostics = yield validationHandler.validateTextDocument(doc);
            const params = {
                context: vscode_languageserver_1.CodeActionContext.create(diagnostics),
                range: undefined,
                textDocument: vscode_languageserver_1.TextDocumentIdentifier.create(testHelper_2.TEST_URI),
            };
            ...
            chai_1.expect(result[0].title).to.be.equal('Convert to boolean');
            chai_1.expect(result[0].edit.changes[doc.uri]).to.exist;
            const edit = result[0].edit.changes[doc.uri];
            chai_1.expect(edit.length).to.be.equal(1);
            chai_1.expect(edit[0]).deep.equal(vscode_languageserver_1.TextEdit.replace(vscode_languageserver_1.Range.create(0, 11, 0, 18), 'false'));
        }));
        it('Should provide convert to boolean action for true', () => __awaiter(void 0, void 0, void 0, function* () {
            languageService.addSchema(testHelper_1.SCHEMA_ID, {
                type: 'object',
                properties: {
                    analytics: {
            ...
            const edit = result[0].edit.changes[doc.uri];
            chai_1.expect(edit.length).to.be.equal(1);
            chai_1.expect(edit[0]).deep.equal(vscode_languageserver_1.TextEdit.replace(vscode_languageserver_1.Range.create(0, 11, 0, 17), 'true'));
        }));
    });
});
